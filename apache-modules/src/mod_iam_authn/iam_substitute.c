...
static void do_pattmatch(ap_filter_t *f, apr_bucket *inb, apr_bucket_brigade *mybb, apr_pool_t *tmp_pool, apr_array_header_t *patterns)
{
	int i;
	int force_quick = 0;
	...
	script = (subst_pattern_t*)patterns->elts;
				...
				if ( script->pattern ) {
					while ( (repl = apr_strmatch(script->pattern, buff, bytes)) ) {
						...
						if ( script->flatten && !force_quick ) {
							 ...
							 * replacement...
							 */
							tmp_b = apr_bucket_transient_create(script->replacement, script->replen, f->r->connection->bucket_alloc);
							/* ... and insert it */
					...
					while ( !ap_regexec(script->regexp, p, AP_MAX_REG_MATCH, regm, 0) ) {
						...
						repl = ap_pregsub(tmp_pool, script->replacement, p, AP_MAX_REG_MATCH, regm);
						if ( script->flatten && !force_quick ) {
							SEDSCAT(s1, s2, tmp_pool, p, regm[0].rm_so, repl);
						} else {
							len = (apr_size_t) (regm[0].rm_eo - regm[0].rm_so);
							SEDRMPATBCKT(b, regm[0].rm_so, tmp_b, len);
							tmp_b = apr_bucket_transient_create(repl, strlen(repl), f->r->connection->bucket_alloc);
							APR_BUCKET_INSERT_BEFORE(b, tmp_b);
						}
						/*
	...
	while ( (b = APR_BRIGADE_FIRST(bb)) && (b != APR_BRIGADE_SENTINEL(bb)) ) {
		if ( APR_BUCKET_IS_EOS(b) ) {
			...
			if ( !APR_BRIGADE_EMPTY(ctx->linebb) ) {
				rv = apr_brigade_pflatten(ctx->linebb, &bflat, &fbytes, ctx->tpool);
				tmp_b = apr_bucket_transient_create(bflat, fbytes, f->r->connection->bucket_alloc);
				do_pattmatch(f, tmp_b, ctx->pattbb, ctx->tpool, patterns);
				...
				while ( bytes > 0 ) {
					...
					if ( nl ) {
						...
						if ( !APR_BRIGADE_EMPTY(ctx->linebb) ) {
							...
							rv = apr_brigade_pflatten(ctx->linebb, &bflat, &fbytes, ctx->tpool);
							b = apr_bucket_transient_create(bflat, fbytes, f->r->connection->bucket_alloc);
							apr_brigade_cleanup(ctx->linebb);
						}
						do_pattmatch(f, b, ctx->pattbb, ctx->tpool, patterns);
						/*
						 ...
						 * iteration.
						 */
						for ( b = APR_BRIGADE_FIRST(ctx->pattbb); b != APR_BRIGADE_SENTINEL(ctx->pattbb); b = APR_BUCKET_NEXT(b) ) {
							num++;
						}
						APR_BRIGADE_CONCAT(ctx->passbb, ctx->pattbb);
						/*
						...
						if ( num > AP_MAX_BUCKETS ) {
							b = apr_bucket_flush_create(f->r->connection->bucket_alloc);
							APR_BRIGADE_INSERT_TAIL(ctx->passbb, b);
...
const char *iam_subst_set_pattern(apr_pool_t *pool, apr_array_header_t *patterns, const char *line)
{
	...
	if ( !is_pattern ) {
		r = ap_pregcomp(pool, from, AP_REG_EXTENDED | (ignore_case ? AP_REG_ICASE : 0));
		if ( !r ) {
			return "Substitute could not compile regex";
		}
	}
	nscript = apr_array_push(patterns);
	...
	if ( is_pattern ) {
		...
		nscript->pattern = apr_strmatch_precompile(pool, from, !ignore_case);
	} else {
		nscript->regexp = r;
	}
